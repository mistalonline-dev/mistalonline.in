...
function initFakeFlow() {
    ...
    setInterval(() => {
        if (Math.random() > 0.7) {
            ...
            statusMsg.className = 'success';
            setTimeout(() => {
                statusMsg.textContent = '💰 Select service & complete payment';
                statusMsg.className = '';
            }, 4000);
        }
    }, 10000);
...
async function placeOrder(event) {
    ...
    if (!service) {
        statusMsg.textContent = '⚠️ Please select a service!';
        statusMsg.className = 'error';
        playVoice('error');
        return;
    }
    ...
    if (amount <= 0) {
        statusMsg.textContent = '⚠️ Invalid amount!';
        statusMsg.className = 'error';
        playVoice('error');
        return;
    }
    ...
    try {
        ...
        // 🔥 UPI Intent URI
        const upiLink = `upi://pay?pa=${UPI_ID}&pn=${encodedName}&am=${amount}.00&cu=INR&tn=${encodedNote}`;

        // 🔥 QR Code URL (fallback if image missing)
        ...
        paymentLink.textContent = upiLink;

        paymentSection.scrollIntoView({ behavior: 'smooth' });

        statusMsg.textContent = '✅ Order created! Scan QR or use UPI to pay.';
        statusMsg.className = 'success';
        orderBtn.textContent = '🔄 New Order';
        playVoice('order_placed');

        await sendOrderEmail(currentOrder);
...
function payWith(app) {
    ...
    const encodedName = encodeURIComponent(MERCHANT_NAME);
    const encodedNote = encodeURIComponent('Order ' + orderId);
    const upiUri = `upi://pay?pa=${UPI_ID}&pn=${encodedName}&am=${amount}.00&cu=INR&tn=${encodedNote}`;

    const appLinks = {
        phonepe: `phonepe://pay?pa=${UPI_ID}&pn=${encodedName}&am=${amount}.00&cu=INR&tn=${encodedNote}`,
        gpay: `tez://upi/pay?pa=${UPI_ID}&pn=${encodedName}&am=${amount}.00&cu=INR&tn=${encodedNote}`,
        paytm: `paytmmp://upi/pay?pa=${UPI_ID}&pn=${encodedName}&am=${amount}.00&cu=INR&tn=${encodedNote}`,
        bhim: `bhim://upi/pay?pa=${UPI_ID}&pn=${encodedName}&am=${amount}.00&cu=INR&tn=${encodedNote}`
    };

    const link = appLinks[app] || upiUri;

    statusMsg.textContent = `🔄 Opening ${app.charAt(0).toUpperCase() + app.slice(1)}...`;
    statusMsg.className = '';
    ...
    try {
        ...
        setTimeout(() => {
            if (document.hidden !== true) {
                ...
                statusMsg.textContent = '🔄 Opening UPI...';
            }
        }, 2000);
    } catch (e) {
        window.location.href = upiUri;
...
// ============================================================
function copyLink() {
    const link = currentOrder.paymentLink || `upi://pay?pa=${UPI_ID}&pn=${encodeURIComponent(MERCHANT_NAME)}&am=${currentOrder.amount}.00&cu=INR&tn=Order%20${currentOrder.orderId}`;
    
    if (navigator.clipboard) {
        navigator.clipboard.writeText(link).then(() => {
            statusMsg.textContent = '✅ UPI link copied! Share with customer.';
            statusMsg.className = 'success';
...
function fallbackCopy(text) {
    ...
    textarea.remove();
    statusMsg.textContent = '✅ UPI link copied!';
    statusMsg.className = 'success';
}
...
screenshotInput.addEventListener('change', function() {
    ...
    reader.onload = function(e) {
        ...
        `;
        statusMsg.textContent = '✅ Screenshot uploaded! Click "Confirm Payment".';
        statusMsg.className = 'success';
    };
...
async function confirmPayment() {
    if (!currentOrder.orderId) {
        statusMsg.textContent = '⚠️ Please place an order first!';
        statusMsg.className = 'error';
        return;
    }
    ...
    try {
        const response = await fetch('https://api.web3forms.com/submit', {
            ...
            body: JSON.stringify({
                access_key: WEB3FORMS_KEY,
                subject: '✅ Payment Confirmed - Order #' + currentOrder.orderId,
        ...
        if (result.success) {
            ...
            statusMsg.textContent = '✅ Order Confirmed! Thank you!';
            statusMsg.className = 'success';
        ...
        statusMsg.textContent = '❌ ' + (error.message || 'Payment confirmation failed. Try again.');
        statusMsg.className = 'error';
    }
}
...
window.addEventListener('load', function() {
    ...
    console.log('📦 Fake Orders Active:', fakeOrders.length);
    console.log('⭐ Fake Reviews Active:', fakeReviews.length);
